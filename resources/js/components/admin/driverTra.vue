      ...
      <div class="_box _b_radious3 _padd20">
        ...
        <div class="_driver_pro">
          ...
          <div class="_driver_pro_details">
            <p class="_driver_pro_name">{{ driverPro.name }}</p>
            <p class="_driver_pro_phone">{{ driverPro.phone }}</p>
          </div>
        </div>

        <div class="_driver_tra">
          <h2 class="_driver_tra_text">Total Transaction : {{ total }}</h2>
          <h2 style="color: green" class="_driver_tra_text">
            Total paid : {{ totalPaid }}
          </h2>
          <h2 style="color: red" class="_driver_tra_text">
            Total due : {{ totalDue }}
          </h2>
        </div>
...
export default {
  ...
  methods: {
    async addTransaction() {
      ...
      this.data.driver_id = this.$route.params.id;

      const res = await this.callApi("post", "/app/add_driverTra", this.data);
  ...
  computed: {
    ...
    inputDue() {
      let inputdueVal = 0;
      this.inputdueVal = this.data.total - this.data.paid;
      console.log("inputDue", this.inputdueVal);
      return inputdueVal;
    },
  },
  ...
  async created() {
    const res = await this.callApi(
      "get",
      `/app/get_driverPro/${this.$route.params.id}`
    );
    if (res.status === 200) {
      this.driverPro = res.data;
      // this.addModal = false
    } else {
      this.swr();
    }
    ...
    const res2 = await this.callApi(
      "get",
      `/app/get_driverTra/${this.$route.params.id}`
    );
    if (res2.status === 200) {
